...
echo "------------------------------------------------------------------------------------"
echo " $THIS_SCRIPT: Setting up basic firewall (ssh-only) at $TARGET_HOST"
echo "------------------------------------------------------------------------------------"
ssh $TARGET "sudo ufw --force disable && sudo ufw default deny incoming && sudo ufw allow ssh && sudo ufw --force enable"
if [ "$?" != "0" ]; then
